...
test = True # batch_size = 3, num_epoch = 2, full dataset
...
omics_encode_dim_dict ={'Mut':[1000,100,50],'Exp':[1000,100,50], # Dr.Chiu:'Mut':[1000,100,50], 'Exp':[500,200,50]
...
Drug_Cell_SelfAttention= "OmicDim" # "OmicDim":50, "SubstructureDim":128, False		
CrossAttention = False			

#需再修改-----------

intermediate_size =512
num_attention_heads = 8 # 要可以整除drug_embedding_feature_size
...
if ESPF is True:
    ...
    dense_layer_dim = sum(omics_encode_dim_dict[omic_type][2] for omic_type in include_omics) + drug_encode_dims[2] # MLPDim
if Drug_Cell_SelfAttention is "OmicDim":
    intermediate_size= 50*4
    drug_embedding_feature_size = 50
    num_attention_heads = 5
    drug_encode_dims =[800,300,100] # 50*50
    dense_layer_dim = sum(omics_encode_dim_dict[omic_type][2] for omic_type in include_omics) + drug_encode_dims[2] # MLPDim
elif Drug_Cell_SelfAttention is "SubstructureDim":
    omics_encode_dim_dict ={'Mut':[2000,1000,128],'Exp':[2000,1000,128], # Dr.Chiu:'Mut':[1000,100,50], 'Exp':[500,200,50]
                        'CN':[100,50,128], 'Eff':[100,50,128], 'Dep':[100,50,128], 'Met':[100,50,128]}
    intermediate_size= 512
    drug_embedding_feature_size = 128
...
continuous = True
learning_rate=1e-05
criterion = Custom_Weighted_LossFunction(loss_type="weighted_MSE", loss_lambda=1.0, regular_type=None, regular_lambda=0.001) #nn.MSELoss()#
