...
from utils.Exp_Drug_Model import Exp_Drug_Model
...
# TCGA_pretrain_weight_path = "/root/Winnie/PDAC/results/Encoder_tcga_mut_33_22_14_best_loss_0.0078.pt"

def run(hyperparameters,filename):
    ...
    exp_encode_dim = hyperparameters["exp_encode_dim"]
    ...
    TCGA_pretrain_weight_path= hyperparameters["TCGA_pretrain_weight_path"]
    ...
    #load data
    data_exp, gene_names,ccl_names  = load_ccl("./CCLE_matched_PRISMauc_PDAC_TCGA_binary_mutation.txt")
    data_drug, drug_names  = load_drug("/root/data/no_Imputation_PRISM_Repurposing_Secondary_Screen_data/MACCS(Secondary_Screen_treatment_info)_union.txt")
    ...
    data_exp, data_drug, data_AUC = repeat_data(data_exp, data_drug, data_AUC_matrix, splitType= splitType, num_ccl=num_ccl, num_drug=num_drug, device=device) 

    data_AUC = data_AUC*valueMultiply
    data_AUC.shape

    print("data_AUC[1]",data_AUC[1])
    print(f'{data_AUC[8].item():.8f}')

    # randomly split by CCL
    # 90% for training(10% for validation) and 10% for testing
    id_unrepeat_train, id_unrepeat_val, id_unrepeat_test, id_unrepeat_train_val, id_train, id_val, id_test= split_id(num_ccl,num_drug,splitType=splitType,repeat=True,kFold=False)


    #create dataset
    seed=42
    torch.manual_seed(seed)
    train_dataset = create_dataset(data_exp, data_drug, data_AUC, id = id_train, batch_size=batch_size)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_dataset = create_dataset(data_exp, data_drug, data_AUC, id = id_val, batch_size=batch_size)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    test_dataset = create_dataset(data_exp, data_drug, data_AUC, id = id_test, batch_size=batch_size)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

    #train
    # Init the neural network  
    torch.manual_seed(seed)
    model = Exp_Drug_Model(exp_encode_dim, drug_encode_dim, activation_func, activation_func_final, dense_layer_dim, device,
                        num_exp_features=data_exp.shape[1], num_drug_features=data_drug.shape[1],
    ...
    train_dataset = TensorDataset(data_exp[id_train],data_drug[id_train], data_AUC[id_train])
