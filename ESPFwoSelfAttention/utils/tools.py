...
def get_data_value_range(data, name='', file=None): 
    ...
    Skewness_data = stats.skew(data, bias=False, nan_policy='raise')
    ...
    if file != None:
        file.write(f"{name} : \n")
        file.write(f"Range: {range_data:.8f}\n")
        file.write(f"Minimum: {min_data:.8f}\n")
        file.write(f"Maximum: {max_data:.8f}\n")
        file.write(f"Mean: {mean_data:.8f}\n")
        file.write(f"Median: {median_data:.8f}\n")
        file.write(f"Standard Deviation: {std_data:.8f}\n")
        file.write(f"Skewness: {Skewness_data:.8f}\n")
        file.write(f"binary data:{TorF}\n")
        file.write(f"-------------------------------------\n")
    else:
        print(f"{name} : ")
        print(f"Range: {range_data:.8f}")
        print(f"Minimum: {min_data:.8f}")
        print(f"Maximum: {max_data:.8f}")
        print(f"Mean: {mean_data:.8f}")
        print(f"Median: {median_data:.8f}")
        print(f"Standard Deviation: {std_data:.8f}")
        print(f"Skewness: {Skewness_data:.8f}")
        print(f"binary data:{TorF}")
        print(f"-------------------------------------")
...
'''
def monitor_gradients_and_weights(model, optimizer):
    for param in model.parameters():
        if param.grad is not None:
            gradient_norm = param.grad.norm().item() # L2 norm (default)
            print(f"Gradient norm: {gradient_norm}")

            if gradient_norm > GRADIENT_CLIP_THRESHOLD:
                print("Clipping gradients...")
                torch.nn.utils.clip_grad_norm_(model.parameters(), GRADIENT_CLIP_THRESHOLD)

        weight_norm = param.data.norm().item() # L2 norm (default)
        print(f"Weight norm: {weight_norm}")

        if weight_norm > WEIGHT_CLIP_THRESHOLD:
            print("Clipping weights...")
            # Implement weight clipping mechanism (e.g., clamp)
            weight_norm.clamp_(-self.weight_clipping_limit, self.weight_clipping_limit)
...
for epoch in range(num_epochs):
    ...
    monitor_gradients_and_weights(model, optimizer)
