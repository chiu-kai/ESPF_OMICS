...
#########
def atom_features(atom): # 輸出78維True/False特徵向量 # 原子的元素符號、degree、幾個氫原子、隱式價電子數、是否為芳香族
...
def smile_to_graph(smile):
    mol = Chem.MolFromSmiles(smile) # mol:分子結構物件
    c_size = mol.GetNumAtoms() # c_size:原子總數
    features = [] # c*78 (原子個數*78維特徵向量)
    for atom in mol.GetAtoms():
        feature = atom_features(atom)# 78維True/False特徵向量
